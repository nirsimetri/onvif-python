...
class TestReceiverWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        port_type = root.find('.//wsdl:portType[@name="ReceiverPort"]', ns)
    ...
    def get_implemented_methods():
        """Get all implemented methods in Receiver class."""
        methods = {}
        for name, method in inspect.getmembers(Receiver, predicate=inspect.isfunction):
    ...
    @patch("onvif.services.receiver.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        receiver = Receiver(xaddr="http://test:80/onvif/Receiver")
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        receiver.GetServiceCapabilities()
        mock_operator_instance.call.assert_called_with("GetServiceCapabilities")

        # Test GetReceivers (no parameters)
        mock_operator_instance.call.reset_mock()
        receiver.GetReceivers()
        mock_operator_instance.call.assert_called_with("GetReceivers")

        # Test GetReceiver (required parameter)
        mock_operator_instance.call.reset_mock()
        receiver.GetReceiver(ReceiverToken="receiver1")
        mock_operator_instance.call.assert_called_with(
            "GetReceiver", ReceiverToken="receiver1"
        )

        # Test CreateReceiver (required parameter)
        mock_operator_instance.call.reset_mock()
        ...
        # Test DeleteReceiver (required parameter)
        mock_operator_instance.call.reset_mock()
        receiver.DeleteReceiver(ReceiverToken="receiver2")
        mock_operator_instance.call.assert_called_with(
            "DeleteReceiver", ReceiverToken="receiver2"
        )

        # Test ConfigureReceiver (multiple required parameters)
        mock_operator_instance.call.reset_mock()
        receiver.ConfigureReceiver(
            ReceiverToken="receiver3",
            Configuration={"Mode": "NeverConnect", "MediaUri": "rtsp://test.com/video"},
        )
        mock_operator_instance.call.assert_called_with(
            "ConfigureReceiver",
            ReceiverToken="receiver3",
        ...
        # Test SetReceiverMode (multiple required parameters)
        mock_operator_instance.call.reset_mock()
        receiver.SetReceiverMode(ReceiverToken="receiver4", Mode="AlwaysConnect")
        mock_operator_instance.call.assert_called_with(
            "SetReceiverMode", ReceiverToken="receiver4", Mode="AlwaysConnect"
        )

        # Test GetReceiverState (required parameter)
        mock_operator_instance.call.reset_mock()
        receiver.GetReceiverState(ReceiverToken="receiver5")
        mock_operator_instance.call.assert_called_with(
            "GetReceiverState", ReceiverToken="receiver5"
        )
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        for test_case in test_cases:
            mock_operator_instance.call.reset_mock()
            method = getattr(receiver, test_case["method"])
