...
class TestMediaSigningWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        # Find all operations in the MediaSigning portType
        port_type = root.find('.//wsdl:portType[@name="MediaSigning"]', ns)
    ...
    def get_implemented_methods():
        """Get all implemented methods in MediaSigning class."""
        methods = {}
        for name, method in inspect.getmembers(
            MediaSigning, predicate=inspect.isfunction
        ):
    ...
    @patch("onvif.services.security.mediasigning.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        # Create MediaSigning instance
        media_signing = MediaSigning(xaddr="http://test:80/onvif/Security")
        ...
        for method_name, method_info in implemented_methods.items():
            ...
            try:
                # Get the method
                method = getattr(media_signing, method_name)
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        # Test AddMediaSigningCertificateAssignment (required parameter)
        media_signing.AddMediaSigningCertificateAssignment(
            CertificationPathID="cert_path_1"
        )
        mock_operator_instance.call.assert_called_with(
            "AddMediaSigningCertificateAssignment", CertificationPathID="cert_path_1"
        )

        # Test RemoveMediaSigningCertificateAssignment (required parameter)
        mock_operator_instance.call.reset_mock()
        media_signing.RemoveMediaSigningCertificateAssignment(
            CertificationPathID="cert_path_2"
        )
        mock_operator_instance.call.assert_called_with(
            "RemoveMediaSigningCertificateAssignment", CertificationPathID="cert_path_2"
        )

        # Test GetAssignedMediaSigningCertificates (no parameters)
        mock_operator_instance.call.reset_mock()
        media_signing.GetAssignedMediaSigningCertificates()
        mock_operator_instance.call.assert_called_with(
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        for test_case in test_cases:
            mock_operator_instance.call.reset_mock()
            method = getattr(media_signing, test_case["method"])
