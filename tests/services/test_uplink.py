...
class TestUplinkWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        ns = {
            ...
            "tul": "http://www.onvif.org/ver10/uplink/wsdl",
        }

        operations = {}

        # Find all operations in portType
        port_type = root.find('.//wsdl:portType[@name="UplinkPort"]', ns)
    ...
    def get_implemented_methods():
        """Get all implemented methods in Uplink class."""
        methods = {}
        for name, method in inspect.getmembers(Uplink, predicate=inspect.isfunction):
    ...
    @patch("onvif.services.uplink.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        # Create Uplink instance
        uplink = Uplink(xaddr="http://test:80/onvif/Uplink")
        ...
        for method_name, method_info in implemented_methods.items():
            ...
            try:
                # Get the method
                method = getattr(uplink, method_name)
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        uplink.GetServiceCapabilities()
        mock_operator_instance.call.assert_called_with("GetServiceCapabilities")

        # Test GetUplinks (no parameters)
        mock_operator_instance.call.reset_mock()
        uplink.GetUplinks()
        mock_operator_instance.call.assert_called_with("GetUplinks")

        # Test SetUplink (required parameter)
        mock_operator_instance.call.reset_mock()
        uplink.SetUplink(
            Configuration={
        ...
        # Test DeleteUplink (required parameter)
        mock_operator_instance.call.reset_mock()
        uplink.DeleteUplink(RemoteAddress="192.168.1.100")
        mock_operator_instance.call.assert_called_with(
            "DeleteUplink", RemoteAddress="192.168.1.100"
        )
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        for test_case in test_cases:
            mock_operator_instance.call.reset_mock()
            method = getattr(uplink, test_case["method"])
