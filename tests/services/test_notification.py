...
class TestNotificationWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        # Find all operations in NotificationProducerBinding (NotificationProducer is imported from WS-Notification)
        binding = root.find('.//wsdl:binding[@name="NotificationProducerBinding"]', ns)
    ...
    def get_implemented_methods():
        """Get all implemented methods in Notification class."""
        methods = {}
        for name, method in inspect.getmembers(
            Notification, predicate=inspect.isfunction
        ):
    ...
    @patch("onvif.services.events.notification.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        notification = Notification(xaddr="http://test:80/onvif/Events")
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        # Test Subscribe with all parameters
        ...
        # Test Subscribe with minimal parameters (all are optional)
        mock_operator_instance.call.reset_mock()
        notification.Subscribe()
        mock_operator_instance.call.assert_called_with(
            "Subscribe",
        ...
        mock_operator_instance.call.reset_mock()
        notification.GetCurrentMessage(Topic="tns1:VideoSource/MotionAlarm")
        mock_operator_instance.call.assert_called_with(
            "GetCurrentMessage", Topic="tns1:VideoSource/MotionAlarm"
        )
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        for test_case in test_cases:
            mock_operator_instance.call.reset_mock()
            method = getattr(notification, test_case["method"])
