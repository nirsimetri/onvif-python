...
class TestPullPointWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        # Find all operations in the PullPointSubscription portType
        port_type = root.find('.//wsdl:portType[@name="PullPointSubscription"]', ns)
    ...
    def get_implemented_methods():
        """Get all implemented methods in PullPoint class."""
        methods = {}
        for name, method in inspect.getmembers(PullPoint, predicate=inspect.isfunction):
    ...
    @patch("onvif.services.events.pullpoint.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        # Create PullPoint instance
        pullpoint = PullPoint(xaddr="http://test:80/onvif/PullPoint")
        ...
        for method_name, method_info in implemented_methods.items():
            ...
            try:
                # Get the method
                method = getattr(pullpoint, method_name)
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        # Test PullMessages
        pullpoint.PullMessages(Timeout="PT1M", MessageLimit=10)
        mock_operator_instance.call.assert_called_with(
            "PullMessages", Timeout="PT1M", MessageLimit=10
        )

        # Test Seek with UtcTime only
        mock_operator_instance.call.reset_mock()
        pullpoint.Seek(UtcTime="2024-01-01T00:00:00Z")
        mock_operator_instance.call.assert_called_with(
            "Seek", UtcTime="2024-01-01T00:00:00Z", Reverse=None
        )

        # Test Seek with Reverse parameter
        mock_operator_instance.call.reset_mock()
        pullpoint.Seek(UtcTime="2024-01-01T12:00:00Z", Reverse=True)
        mock_operator_instance.call.assert_called_with(
            "Seek", UtcTime="2024-01-01T12:00:00Z", Reverse=True
        )

        # Test SetSynchronizationPoint (no parameters)
        mock_operator_instance.call.reset_mock()
        pullpoint.SetSynchronizationPoint()
        mock_operator_instance.call.assert_called_with("SetSynchronizationPoint")

        # Test Unsubscribe (no parameters)
        mock_operator_instance.call.reset_mock()
        pullpoint.Unsubscribe()
        mock_operator_instance.call.assert_called_with("Unsubscribe")
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        for test_case in test_cases:
            mock_operator_instance.call.reset_mock()
            method = getattr(pullpoint, test_case["method"])
