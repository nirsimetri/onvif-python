...
class TestAuthorizationServerWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        # Find all operations in the AuthorizationServer portType
        port_type = root.find('.//wsdl:portType[@name="AuthorizationServer"]', ns)
    ...
    def get_implemented_methods():
        """Get all implemented methods in AuthorizationServer class."""
        methods = {}
        for name, method in inspect.getmembers(
            AuthorizationServer, predicate=inspect.isfunction
        ):
    ...
    @patch("onvif.services.security.authorizationserver.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        auth_server = AuthorizationServer(
            xaddr="http://test:80/onvif/AuthorizationServer"
        )
        ...
        for method_name, method_info in implemented_methods.items():
            ...
            try:
                # Get the method
                method = getattr(auth_server, method_name)
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        auth_server = AuthorizationServer(xaddr="http://test:80/onvif/Security")

        # Test GetAuthorizationServerConfigurations (optional parameter)
        auth_server.GetAuthorizationServerConfigurations()
        mock_operator_instance.call.assert_called_with(
            "GetAuthorizationServerConfigurations", Token=None
        )

        # Test GetAuthorizationServerConfigurations with Token
        mock_operator_instance.call.reset_mock()
        auth_server.GetAuthorizationServerConfigurations(Token="config1")
        mock_operator_instance.call.assert_called_with(
            "GetAuthorizationServerConfigurations", Token="config1"
        )

        # Test CreateAuthorizationServerConfiguration (required parameter)
        mock_operator_instance.call.reset_mock()
        auth_server.CreateAuthorizationServerConfiguration(
            Configuration={
                "Token": "auth_server_1",
                "Name": "OAuth Server",
        ...
        # Test SetAuthorizationServerConfiguration (required parameter)
        mock_operator_instance.call.reset_mock()
        auth_server.SetAuthorizationServerConfiguration(
            Configuration={
                "Token": "auth_server_2",
        ...
        # Test DeleteAuthorizationServerConfiguration (required parameter)
        mock_operator_instance.call.reset_mock()
        auth_server.DeleteAuthorizationServerConfiguration(Token="auth_server_3")
        mock_operator_instance.call.assert_called_with(
            "DeleteAuthorizationServerConfiguration", Token="auth_server_3"
        )
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        for test_case in test_cases:
            mock_operator_instance.call.reset_mock()
            method = getattr(auth_server, test_case["method"])
