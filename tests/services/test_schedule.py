...
class TestScheduleWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        port_type = root.find('.//wsdl:portType[@name="SchedulePort"]', ns)
    ...
    def get_implemented_methods():
        """Get all implemented methods in Schedule class."""
        methods = {}
        for name, method in inspect.getmembers(Schedule, predicate=inspect.isfunction):
    ...
    @patch("onvif.services.schedule.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        schedule = Schedule(xaddr="http://test:80/onvif/Schedule")
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        schedule.GetServiceCapabilities()
        mock_operator_instance.call.assert_called_with("GetServiceCapabilities")

        # Test GetScheduleState (required parameter)
        mock_operator_instance.call.reset_mock()
        schedule.GetScheduleState(Token="schedule1")
        mock_operator_instance.call.assert_called_with(
            "GetScheduleState", Token="schedule1"
        )

        # Test GetScheduleInfo (required parameter)
        mock_operator_instance.call.reset_mock()
        schedule.GetScheduleInfo(Token="schedule2")
        mock_operator_instance.call.assert_called_with(
            "GetScheduleInfo", Token="schedule2"
        )

        # Test GetScheduleInfoList (optional parameters)
        mock_operator_instance.call.reset_mock()
        schedule.GetScheduleInfoList(Limit=10, StartReference="ref1")
        mock_operator_instance.call.assert_called_with(
            "GetScheduleInfoList", Limit=10, StartReference="ref1"
        )

        # Test GetSchedules (required parameter)
        mock_operator_instance.call.reset_mock()
        schedule.GetSchedules(Token="schedule3")
        mock_operator_instance.call.assert_called_with(
            "GetSchedules", Token="schedule3"
        )

        # Test CreateSchedule (required parameter)
        mock_operator_instance.call.reset_mock()
        ...
        # Test SetSchedule (required parameter)
        mock_operator_instance.call.reset_mock()
        schedule.SetSchedule(
            Schedule={"Token": "schedule4", "Name": "Updated Schedule"}
        )
        mock_operator_instance.call.assert_called_with(
            "SetSchedule", Schedule={"Token": "schedule4", "Name": "Updated Schedule"}
        )

        # Test DeleteSchedule (required parameter)
        mock_operator_instance.call.reset_mock()
        schedule.DeleteSchedule(Token="schedule5")
        mock_operator_instance.call.assert_called_with(
            "DeleteSchedule", Token="schedule5"
        )

        # Test GetSpecialDayGroupInfo (required parameter)
        mock_operator_instance.call.reset_mock()
        schedule.GetSpecialDayGroupInfo(Token="daygroup1")
        mock_operator_instance.call.assert_called_with(
            "GetSpecialDayGroupInfo", Token="daygroup1"
        )

        # Test CreateSpecialDayGroup (required parameter)
        mock_operator_instance.call.reset_mock()
        schedule.CreateSpecialDayGroup(
        ...
        # Test DeleteSpecialDayGroup (required parameter)
        mock_operator_instance.call.reset_mock()
        schedule.DeleteSpecialDayGroup(Token="daygroup2")
        mock_operator_instance.call.assert_called_with(
            "DeleteSpecialDayGroup", Token="daygroup2"
        )
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        for test_case in test_cases:
            mock_operator_instance.call.reset_mock()
            method = getattr(schedule, test_case["method"])
