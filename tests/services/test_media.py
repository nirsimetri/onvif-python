...
class TestMediaWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        port_type = root.find('.//wsdl:portType[@name="Media"]', ns)
    ...
    def get_implemented_methods():
        """Get all implemented methods in Media class."""
        methods = {}
    ...
    @patch("onvif.services.media.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        media = Media(xaddr="http://test:80/onvif/Media")
        ...
        for method_name, method_info in implemented_methods.items():
            ...
            try:
                # Get the method
                method = getattr(media, method_name)
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        # Test GetProfiles (no parameters)
        mock_operator_instance.call.reset_mock()
        media.GetProfiles()
        mock_operator_instance.call.assert_called_with("GetProfiles")

        # Test GetProfile (required parameter)
        mock_operator_instance.call.reset_mock()
        media.GetProfile(ProfileToken="profile1")
        mock_operator_instance.call.assert_called_with(
            "GetProfile", ProfileToken="profile1"
        )

        # Test CreateProfile (required + optional parameter)
        mock_operator_instance.call.reset_mock()
        media.CreateProfile(Name="TestProfile", Token="token1")
        mock_operator_instance.call.assert_called_with(
            "CreateProfile", Name="TestProfile", Token="token1"
        )
        ...
        mock_operator_instance.call.reset_mock()
        media.GetSnapshotUri(ProfileToken="profile3")
        mock_operator_instance.call.assert_called_with(
            "GetSnapshotUri", ProfileToken="profile3"
        )

        # Test AddVideoEncoderConfiguration (multiple required parameters)
        mock_operator_instance.call.reset_mock()
        media.AddVideoEncoderConfiguration(
            ProfileToken="profile4", ConfigurationToken="config1"
        )
        mock_operator_instance.call.assert_called_with(
        ...
        # Test SetVideoEncoderConfiguration (multiple required parameters)
        mock_operator_instance.call.reset_mock()
        ...
        media.GetVideoEncoderConfigurationOptions(
            ConfigurationToken="config3", ProfileToken="profile5"
        )
        mock_operator_instance.call.assert_called_with(
            "GetVideoEncoderConfigurationOptions",
            ConfigurationToken="config3",
            ProfileToken="profile5",
        )

        # Test DeleteProfile (required parameter)
        mock_operator_instance.call.reset_mock()
        media.DeleteProfile(ProfileToken="profile6")
        mock_operator_instance.call.assert_called_with(
            "DeleteProfile", ProfileToken="profile6"
        )
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        test_cases = [
            ...
            {
                "method": "RemovePTZConfiguration",
                "params": {"ProfileToken": "prof8"},
            },
            {
                "method": "GetVideoSourceModes",
        ...
        for test_case in test_cases:
            mock_operator_instance.call.reset_mock()
            method = getattr(media, test_case["method"])
