...
class TestAdvancedSecurityWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        # Find all operations in the main AdvancedSecurityService portType
        port_type = root.find('.//wsdl:portType[@name="AdvancedSecurityService"]', ns)
    ...
    def get_implemented_methods():
        """Get all implemented methods in AdvancedSecurity class."""
        methods = {}
        for name, method in inspect.getmembers(
            AdvancedSecurity, predicate=inspect.isfunction
        ):
    ...
    @patch("onvif.services.security.advancedsecurity.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        # Create AdvancedSecurity instance
        adv_security = AdvancedSecurity(xaddr="http://test:80/onvif/Security")
        ...
        for method_name, method_info in implemented_methods.items():
            ...
            try:
                # Get the method
                method = getattr(adv_security, method_name)
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        adv_security.GetServiceCapabilities()
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        assert (
            actual_call[0][0] == "GetServiceCapabilities"
        ), "Operation name mismatch for GetServiceCapabilities"

    def test_wsdl_structure_info(self):
        """Informational test to show WSDL structure."""
        wsdl_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
            "onvif",
            "wsdl",
        ...
        for pt in port_types:
            pt_name = pt.get("name")
            operations = [op.get("name") for op in pt.findall("wsdl:operation", ns)]
            port_type_info[pt_name] = operations

        print(f"\nAdvancedSecurity WSDL contains {len(port_types)} portTypes:")
        for pt_name, ops in port_type_info.items():
            print(f"  - {pt_name}: {len(ops)} operations")

        # Note: Current implementation only covers AdvancedSecurityService portType
        # Other portTypes (JWT, Keystore, TLSServer, Dot1X, MediaSigning, AuthorizationServer)
        # may need separate implementation classes or extended methods
