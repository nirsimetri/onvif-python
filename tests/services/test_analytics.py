...
class TestAnalyticsWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        port_type = root.find('.//wsdl:portType[@name="AnalyticsEnginePort"]', ns)
    ...
    def get_implemented_methods():
        """Get all implemented methods in Analytics class."""
        methods = {}
        for name, method in inspect.getmembers(Analytics, predicate=inspect.isfunction):
    ...
    @patch("onvif.services.analytics.analytics.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        analytics = Analytics(xaddr="http://test:80/onvif/Analytics")
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        # Test GetSupportedAnalyticsModules (required parameter)
        mock_operator_instance.call.reset_mock()
        analytics.GetSupportedAnalyticsModules(ConfigurationToken="token1")
        mock_operator_instance.call.assert_called_with(
            "GetSupportedAnalyticsModules", ConfigurationToken="token1"
        )

        # Test CreateAnalyticsModules (multiple required parameters)
        mock_operator_instance.call.reset_mock()
        analytics.CreateAnalyticsModules(
            ConfigurationToken="token1", AnalyticsModule={"Name": "module1"}
        )
        mock_operator_instance.call.assert_called_with(
        ...
        # Test GetAnalyticsModuleOptions (required + optional parameter)
        mock_operator_instance.call.reset_mock()
        analytics.GetAnalyticsModuleOptions(
            ConfigurationToken="token1", Type="SomeType"
        )
        mock_operator_instance.call.assert_called_with(
        ...
        # Test GetSupportedMetadata (optional parameter only)
        mock_operator_instance.call.reset_mock()
        analytics.GetSupportedMetadata(Type="MetadataType")
        mock_operator_instance.call.assert_called_with(
            "GetSupportedMetadata", Type="MetadataType"
        )

        # Test GetSupportedMetadata without parameter
        mock_operator_instance.call.reset_mock()
        analytics.GetSupportedMetadata()
        mock_operator_instance.call.assert_called_with(
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        for test_case in test_cases:
            mock_operator_instance.call.reset_mock()
            method = getattr(analytics, test_case["method"])
