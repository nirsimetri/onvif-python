...
def test_accesscontrol_import():
    assert AccessControl is not None


class TestAccessControlWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        port_type = root.find('.//wsdl:portType[@name="PACSPort"]', ns)
    ...
    def get_implemented_methods():
        ...
        for name, method in inspect.getmembers(
            AccessControl, predicate=inspect.isfunction
        ):
    ...
    @patch("onvif.services.accesscontrol.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        access_control = AccessControl(xaddr="http://test:80/onvif/AccessControl")
        ...
        for method_name, method_info in implemented_methods.items():
            ...
            try:
                # Get the method
                method = getattr(access_control, method_name)
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        access_control.GetServiceCapabilities()
        mock_operator_instance.call.assert_called_with("GetServiceCapabilities")

        # Test GetAccessPointInfo (required parameter)
        mock_operator_instance.call.reset_mock()
        access_control.GetAccessPointInfo(Token="token123")
        mock_operator_instance.call.assert_called_with(
            "GetAccessPointInfo", Token="token123"
        )

        # Test GetAccessPointInfoList (optional parameters)
        mock_operator_instance.call.reset_mock()
        access_control.GetAccessPointInfoList(Limit=10, StartReference="ref1")
        mock_operator_instance.call.assert_called_with(
            "GetAccessPointInfoList", Limit=10, StartReference="ref1"
        )

        # Test GetAccessPointInfoList without parameters
        mock_operator_instance.call.reset_mock()
        access_control.GetAccessPointInfoList()
        mock_operator_instance.call.assert_called_with(
            "GetAccessPointInfoList", Limit=None, StartReference=None
        )

        # Test CreateAccessPoint (required parameter)
        mock_operator_instance.call.reset_mock()
        access_control.CreateAccessPoint(AccessPoint={"Token": "ap1", "Name": "Door1"})
        mock_operator_instance.call.assert_called_with(
            "CreateAccessPoint", AccessPoint={"Token": "ap1", "Name": "Door1"}
        )

        # Test SetAccessPointAuthenticationProfile (multiple required parameters)
        mock_operator_instance.call.reset_mock()
        access_control.SetAccessPointAuthenticationProfile(
            Token="token123", AuthenticationProfileToken="auth123"
        )
        mock_operator_instance.call.assert_called_with(
        ...
        # Test ExternalAuthorization (mixed required and optional parameters)
        mock_operator_instance.call.reset_mock()
        access_control.ExternalAuthorization(
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        for test_case in test_cases:
            mock_operator_instance.call.reset_mock()
            method = getattr(access_control, test_case["method"])
