...
class TestImagingWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        port_type = root.find('.//wsdl:portType[@name="ImagingPort"]', ns)
    ...
    def get_implemented_methods():
        """Get all implemented methods in Imaging class."""
        methods = {}
        for name, method in inspect.getmembers(Imaging, predicate=inspect.isfunction):
    ...
    @patch("onvif.services.imaging.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        imaging = Imaging(xaddr="http://test:80/onvif/Imaging")
        ...
        for method_name, method_info in implemented_methods.items():
            ...
            try:
                # Get the method
                method = getattr(imaging, method_name)
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        # Test GetImagingSettings (required parameter)
        mock_operator_instance.call.reset_mock()
        imaging.GetImagingSettings(VideoSourceToken="source1")
        mock_operator_instance.call.assert_called_with(
            "GetImagingSettings", VideoSourceToken="source1"
        )

        # Test SetImagingSettings (required + optional parameters)
        mock_operator_instance.call.reset_mock()
        ...
        # Test GetOptions (required parameter)
        mock_operator_instance.call.reset_mock()
        imaging.GetOptions(VideoSourceToken="source3")
        mock_operator_instance.call.assert_called_with(
            "GetOptions", VideoSourceToken="source3"
        )

        # Test Move (multiple required parameters)
        mock_operator_instance.call.reset_mock()
        imaging.Move(VideoSourceToken="source4", Focus={"Absolute": 0.5})
        mock_operator_instance.call.assert_called_with(
            "Move", VideoSourceToken="source4", Focus={"Absolute": 0.5}
        )

        # Test Stop (required parameter)
        mock_operator_instance.call.reset_mock()
        imaging.Stop(VideoSourceToken="source5")
        mock_operator_instance.call.assert_called_with(
            "Stop", VideoSourceToken="source5"
        )

        # Test GetStatus (required parameter)
        mock_operator_instance.call.reset_mock()
        imaging.GetStatus(VideoSourceToken="source6")
        mock_operator_instance.call.assert_called_with(
            "GetStatus", VideoSourceToken="source6"
        )

        # Test GetMoveOptions (required parameter)
        mock_operator_instance.call.reset_mock()
        imaging.GetMoveOptions(VideoSourceToken="source7")
        mock_operator_instance.call.assert_called_with(
            "GetMoveOptions", VideoSourceToken="source7"
        )

        # Test GetPresets (required parameter)
        mock_operator_instance.call.reset_mock()
        imaging.GetPresets(VideoSourceToken="source8")
        mock_operator_instance.call.assert_called_with(
            "GetPresets", VideoSourceToken="source8"
        )

        # Test GetCurrentPreset (required parameter)
        mock_operator_instance.call.reset_mock()
        imaging.GetCurrentPreset(VideoSourceToken="source9")
        mock_operator_instance.call.assert_called_with(
            "GetCurrentPreset", VideoSourceToken="source9"
        )

        # Test SetCurrentPreset (multiple required parameters)
        mock_operator_instance.call.reset_mock()
        imaging.SetCurrentPreset(VideoSourceToken="source10", PresetToken="preset1")
        mock_operator_instance.call.assert_called_with(
            "SetCurrentPreset", VideoSourceToken="source10", PresetToken="preset1"
        )
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        for test_case in test_cases:
            mock_operator_instance.call.reset_mock()
            method = getattr(imaging, test_case["method"])
