...
class TestJWTWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        # Find all operations in the JWT portType
        port_type = root.find('.//wsdl:portType[@name="JWT"]', ns)
    ...
    def get_implemented_methods():
        """Get all implemented methods in JWT class."""
        methods = {}
        for name, method in inspect.getmembers(JWT, predicate=inspect.isfunction):
    ...
    @patch("onvif.services.security.jwt.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        # Create JWT instance
        jwt = JWT(xaddr="http://test:80/onvif/Security")
        ...
        for method_name, method_info in implemented_methods.items():
            ...
            try:
                # Get the method
                method = getattr(jwt, method_name)
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        # Test GetJWTConfiguration (no parameters)
        jwt.GetJWTConfiguration()
        mock_operator_instance.call.assert_called_with("GetJWTConfiguration")

        # Test SetJWTConfiguration (required parameter)
        mock_operator_instance.call.reset_mock()
        jwt.SetJWTConfiguration(
            Configuration={
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        for test_case in test_cases:
            mock_operator_instance.call.reset_mock()
            method = getattr(jwt, test_case["method"])
