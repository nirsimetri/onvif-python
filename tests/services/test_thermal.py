...
class TestThermalWSDLCompliance:
    ...
    def get_wsdl_operations():
        ...
        port_type = root.find('.//wsdl:portType[@name="ThermalPort"]', ns)
    ...
    def get_implemented_methods():
        """Get all implemented methods in Thermal class."""
        methods = {}
        for name, method in inspect.getmembers(Thermal, predicate=inspect.isfunction):
    ...
    @patch("onvif.services.thermal.ONVIFOperator")
    def test_operator_call_usage(self, mock_operator_class):
        ...
        # Create Thermal instance
        thermal = Thermal(xaddr="http://test:80/onvif/Thermal")
        ...
        for method_name, method_info in implemented_methods.items():
            ...
            try:
                # Get the method
                method = getattr(thermal, method_name)
    ...
    def test_specific_methods_implementation(self, mock_operator_class):
        ...
        thermal.GetServiceCapabilities()
        mock_operator_instance.call.assert_called_with("GetServiceCapabilities")

        # Test GetConfigurationOptions (required parameter)
        mock_operator_instance.call.reset_mock()
        thermal.GetConfigurationOptions(VideoSourceToken="source1")
        mock_operator_instance.call.assert_called_with(
            "GetConfigurationOptions", VideoSourceToken="source1"
        )

        # Test GetConfiguration (required parameter)
        mock_operator_instance.call.reset_mock()
        thermal.GetConfiguration(VideoSourceToken="source2")
        mock_operator_instance.call.assert_called_with(
            "GetConfiguration", VideoSourceToken="source2"
        )

        # Test GetConfigurations (no parameters)
        mock_operator_instance.call.reset_mock()
        thermal.GetConfigurations()
        mock_operator_instance.call.assert_called_with("GetConfigurations")

        # Test SetConfiguration (required parameters)
        mock_operator_instance.call.reset_mock()
        thermal.SetConfiguration(
        ...
        # Test GetRadiometryConfigurationOptions (required parameter)
        mock_operator_instance.call.reset_mock()
        thermal.GetRadiometryConfigurationOptions(VideoSourceToken="source4")
        mock_operator_instance.call.assert_called_with(
            "GetRadiometryConfigurationOptions", VideoSourceToken="source4"
        )

        # Test GetRadiometryConfiguration (required parameter)
        mock_operator_instance.call.reset_mock()
        thermal.GetRadiometryConfiguration(VideoSourceToken="source5")
        mock_operator_instance.call.assert_called_with(
            "GetRadiometryConfiguration", VideoSourceToken="source5"
        )

        # Test SetRadiometryConfiguration (required parameters)
        mock_operator_instance.call.reset_mock()
        thermal.SetRadiometryConfiguration(
            VideoSourceToken="source6",
            Configuration={"ReflectedAmbientTemperature": 20.0, "Emissivity": 0.95},
        )
        mock_operator_instance.call.assert_called_with(
    ...
    def test_parameter_forwarding(self, mock_operator_class):
        ...
        for test_case in test_cases:
            mock_operator_instance.call.reset_mock()
            method = getattr(thermal, test_case["method"])
